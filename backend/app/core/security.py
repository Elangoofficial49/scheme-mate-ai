...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    ...
    try:
        if pwd_context.verify(plain_password, hashed_password):
            return True
        if plain_password.strip() != plain_password and pwd_context.verify(plain_password.strip(), hashed_password):
            return True
        return False
    except Exception:
        # Fallback to simple comparison if hash format differs or plaintext in testing/dev
