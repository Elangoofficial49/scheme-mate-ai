...
class _BusinessProfileFormScreenState extends State<BusinessProfileFormScreen> {
  ...
  final _companyNameController =
      TextEditingController(text: "Sri Lakshmi Textiles");
  final _businessDescController =
      TextEditingController(text: "Garment manufacturing and stitching unit");
    ...
    if (res["success"] == true && res["data"] != null && mounted) {
      ...
      setState(() {
        if (data["company_name"] != null &&
            (data["company_name"] as String).isNotEmpty) {
          _companyNameController.text = data["company_name"];
        }
        if (data["business_description"] != null &&
            (data["business_description"] as String).isNotEmpty) {
        ...
        if (data["source_of_income"] != null &&
            _incomeSources.contains(data["source_of_income"])) {
        ...
        if (data["category"] != null &&
          ...
          if (_categories.contains(cat)) {
            ...
            final match = _categories.firstWhere(
              (c) =>
                  c.toLowerCase().startsWith(cat.toLowerCase()) ||
                  cat
                      .toLowerCase()
                      .startsWith(c.split(' ').first.toLowerCase()),
          ...
          if (_states.contains(st)) {
            ...
            final match = _states.firstWhere(
              (s) =>
                  ...
                  s.toLowerCase().contains(st.toLowerCase()),
              orElse: () => "Other (Specify manually)",
            );
            if (match == "Other (Specify manually)") {
        ...
        if (data["business_type"] != null &&
          ...
          if (_businessTypes.contains(bType)) {
            ...
            final match = _businessTypes.firstWhere(
              (bt) =>
                  bt.toLowerCase().contains(bType.toLowerCase()) ||
                  bType
                      .toLowerCase()
                      .contains(bt.split(' ').first.toLowerCase()),
              orElse: () => "Other (Specify manually)",
            );
            if (match == "Other (Specify manually)") {
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                title: const Text("Browse System Folders / Files",
                    style:
                        TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                subtitle: const Text(
                    "Select image or certificate file from your local storage/folder"),
                onTap: () {
                  Navigator.pop(ctx);
              ...
              ListTile(
                ...
                title: const Text("Use Sample Mock Certificate",
                    style:
                        TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                subtitle: const Text(
                    "Quickly test OCR engine with pre-formatted sample certificate"),
                onTap: () {
                  Navigator.pop(ctx);
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                "✅ Document '$fileName' scanned successfully! Number: '$extractedNum'"),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text("Unable to scan file: $e"),
              backgroundColor: Colors.red),
        );
      }
    }
  ...
  void _scanMockCertificateData() async {
    ...
    } else if (certTypeClean.contains("community") ||
        certTypeClean.contains("caste")) {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              "✅ Sample certificate scanned! Number '$extractedNum' extracted via OCR."),
          backgroundColor: AppTheme.successGreen,
        ),
      );
    }
  ...
  void _submitProfileForm() async {
    ...
    final finalCategory = _selectedCategory == "Other (Specify manually)"
        ? (_customCasteController.text.trim().isNotEmpty
            ? _customCasteController.text.trim()
            : "Other")
        : _selectedCategory;
    final finalBusinessType =
        _selectedBusinessType == "Other (Specify manually)"
    ...
    final finalState = _selectedState == "Other (Specify manually)"
        ? (_customStateController.text.trim().isNotEmpty
    ...
    final payload = {
      ...
      "annual_income":
          double.tryParse(_annualIncomeController.text.trim()) ?? 150000.0,
      ...
      "has_udyam_registration":
          hasCert && _selectedCertificateType.contains("Udyam"),
    };
    ...
    if (res["success"] == true && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              "✅ Profile saved! Fetching matching scheme recommendations..."),
          backgroundColor: AppTheme.successGreen,
        ),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(res["error"]?["message"] ??
              "Failed to save profile. Please check connections."),
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // 1. Company Name
                      Text(context.tr("field_company_name"),
                          style: const TextStyle(
                      ...
                      Text(context.tr("field_business_desc"),
                          style: const TextStyle(
                              fontSize: 15, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 6),
                      TextFormField(
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(context.tr("field_business_type"),
                                    style: const TextStyle(
                                        fontSize: 14,
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(context.tr("field_annual_income"),
                                    style: const TextStyle(
                                        fontSize: 14,
                                        fontWeight: FontWeight.bold)),
                                const SizedBox(height: 6),
                      ...
                      // 5. Source of Income
                      Text(context.tr("field_income_source"),
                          style: const TextStyle(
                              fontSize: 15, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 6),
                      DropdownButtonFormField<String>(
                      ...
                      Text(context.tr("field_category_caste"),
                          style: const TextStyle(
                              fontSize: 15, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 6),
                      DropdownButtonFormField<String>(
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                            color: _certificateNumberController.text
                                    .trim()
                                    .isNotEmpty
                                ? AppTheme.successGreen
                                : Colors.grey.shade300,
                            width: _certificateNumberController.text
                                    .trim()
                                    .isNotEmpty
                                ? 1.5
                                : 1,
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(context.tr("select_cert_type"),
                                style: const TextStyle(
                            ...
                            TextFormField(
                              ...
                              decoration: InputDecoration(
                                hintText: _getCertificatePlaceholder(
                                    _selectedCertificateType),
                                prefixIcon: const Icon(
                                    Icons.confirmation_number_outlined),
                                suffixIcon: _certificateNumberController.text
                                        .trim()
                                        .isNotEmpty
                            ...
                            if (_certificateNumberController.text
                                .trim()
                                .isNotEmpty) ...[
                              const SizedBox(height: 10),
                              Container(
